...
class ExportService {
      ...
      for (String field in fields) {
        switch (field) {
          ...
          case 'Priority':
            ...
            break;
          case 'Category':
            row.add(task.category);
            break;
          case 'Subtasks':
            final subtasksTitles =
                task.subtasks?.map((s) => s.title ?? 'N/A').join('; ') ?? '';
  ...
  Future<String> savePdfFile(Uint8List pdfBytes) async {
    final directoryPath = await _getPublicDownloadPath();
    final fileName =
        'tasks_export_${DateTime.now().millisecondsSinceEpoch}.pdf';
    final path = '$directoryPath/$fileName';
    final file = File(path);
        ...
        build: (pw.Context context) {
          ...
          final Map<int, pw.TableColumnWidth> dynamicColumnWidths =
              Map.fromEntries(
                List.generate(
                  headers.length,
                  (index) => MapEntry(index, const pw.FlexColumnWidth(1)),
                ),
              );
