...
class _CompactCalendarViewState extends State<CompactCalendarView> {
  ...
  void _onDaySelected(DateTime selectedDay) {
    setState(() {
      ...
      _focusedDay =
          selectedDay; // Jab din select ho toh focused month bhi set ho
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<TaskProvider>(
      builder: (context, taskProvider, child) {
        ...
        return Scaffold(
          backgroundColor:
              theme.scaffoldBackgroundColor, // FIX 7: Background color theme se
          appBar: AppBar(
            title: Text(
              'Calendar View',
              style: TextStyle(color: textColor),
            ), // FIX 8: Text color theme se
              ...
              children: [
                ...
                Padding(
                  ...
                  child: CalendarGrid(
                    ...
                    onDaySelected:
                        _onDaySelected, // FIX 8: onDaySelected pass karein
                ...
                Container(
                  constraints: const BoxConstraints(
                    minHeight: 200,
                  ), // Min height diya taakay UI na tootay
                  margin: const EdgeInsets.symmetric(horizontal: 16),
                  decoration: BoxDecoration(
                    color:
                        surfaceColor, // FIX 13: Card background color theme se
                    borderRadius: BorderRadius.circular(10),
                  ),
                  child: tasksForSelectedDay.isEmpty
                      ? Padding(
                          // FIX 14: Padding widget ko const hata kar wrap karein
  ...
  Widget build(BuildContext context) {
    ...
    for (int day = 1; day <= daysInMonth; day++) {
      ...
      dayWidgets.add(
        GestureDetector(
          onTap: () =>
              onDaySelected(currentDay), // FIX 16: On tap select karein
...
class CalendarDayCell extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final Color accentColor = theme.colorScheme.secondary;
    final Color textColor = theme.colorScheme.onBackground;
    final Color selectedTextColor = theme.brightness == Brightness.dark
        ? primaryDark
        : textLight; // Selected day par Dark BG hai, isliye FG light hona chahiye.
    ...
    if (isSelected) {
      dayBgColor =
          accentColor; // FIX 22: Selected day ka background accent color
        ...
        Container(
          ...
          decoration: BoxDecoration(
            ...
            border: isToday || isSelected
                ? Border.all(color: dayBorderColor, width: 1)
                : null,
          ),
          child: Text(
            '$day',
        ...
        if (hasTasks) // FIX 21: taskCount ke bajaye hasTasks use karein
          Container(
            ...
            decoration: const BoxDecoration(
              color:
                  priorityHigh, // Use priorityHigh for the dot color (constant)
              shape: BoxShape.circle,
            ),
          ),
      ],
