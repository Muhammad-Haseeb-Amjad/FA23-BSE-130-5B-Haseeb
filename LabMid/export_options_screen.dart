...
class _ExportOptionsScreenState extends State<ExportOptionsScreen> {
  // Mock Date Range
  TextEditingController startDateController = TextEditingController(text: 'YYYY-MM-DD');
  TextEditingController endDateController = TextEditingController(text: 'YYYY-MM-DD');
  ...
  final List<String> availableFields = const [
    ...
    // 'Priority', // Agar Task model mein priority enum/text hai
  ];
  ...
  void _showDatePicker(TextEditingController controller) async {
    ...
    if (pickedDate != null) {
      setState(() {
        controller.text = pickedDate.toString().split(' ')[0]; // Format to YYYY-MM-DD
      });
    }
  }

  void _navigateToPreview(List<Task> allTasks, ExportOptionsProvider provider) async {
    ...
    // 3. Generate CSV/JSON content (CSV for now)
    String csvContent = ExportService().generateCsv(tasksToExport, fieldsToInclude);
  ...
  Widget build(BuildContext context) {
    ...
    final selectedFields = exportProvider.options.selectedFields; // Provider se current selected fields
                  ...
                  child: SingleChildScrollView(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Text('Select Date Range', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: textColor)),
                        const SizedBox(height: 8),
                        _buildDateField('Start Date', startDateController, accentColor, textColor),
                        const SizedBox(height: 12),
                        _buildDateField('End Date', endDateController, accentColor, textColor),
                        ...
                        // ✅ FIX 1: Hardcoded color ki jagah textColor use kiya gaya
                        Text('Include Fields in Export', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: textColor)),
                        ...availableFields.map((key) {
                          ...
                          return CheckboxListTile(
                            ...
                            checkColor: theme.colorScheme.onSecondary, // Usually black or white text on accent color
                ...
                Padding(
                  ...
                  child: SizedBox(
                    ...
                    child: ElevatedButton(
                      ...
                      onPressed: () => _navigateToPreview(allTasks, exportProvider),
  ...
  Widget _buildDateField(String label, TextEditingController controller, Color accentColor, Color textColor) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
          ...
          child: IgnorePointer(
            child: TextField(
              ...
              decoration: InputDecoration(
                ...
                suffixIcon: Icon(Icons.calendar_today, color: accentColor), // ✅ ACCENT COLOR FIX
              ),
            ),
          ),
        ),
