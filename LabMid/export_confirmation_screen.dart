...
class ExportConfirmationScreen extends StatelessWidget {
    ...
    if (tasksToExport.isEmpty) {
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Error: No tasks available to export. Check TaskProvider.')),
        );
      }
  ...
  Widget build(BuildContext context) {
    // Theme colors
    final Color textLight = Theme.of(context).colorScheme.onBackground;
    final Color accentGreen = Theme.of(context).colorScheme.secondary;
    final Color primaryDark = Theme.of(context).colorScheme.primary;
    final Color priorityMedium = Colors.orange;

    return Scaffold(
      appBar: AppBar(
        title: const Text('3. Confirm & Export'),
      ),
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            // --- Export Summary ---
            _buildSummaryTile('Format', options.format),
            _buildSummaryTile('Tasks Included', 'All tasks'),

            const SizedBox(height: 20),
            Text(
              'Selected Fields:',
              style: TextStyle(color: accentGreen, fontWeight: FontWeight.bold, fontSize: 14),
            ),
            const SizedBox(height: 10),
            Wrap(
              spacing: 8.0,
              runSpacing: 8.0,
              children: options.selectedFields.map((field) => Chip(
                label: Text(field, style: TextStyle(color: primaryDark)),
                backgroundColor: priorityMedium.withOpacity(0.8),
              )).toList(),
            ),
  ...
  Widget _buildSummaryTile(String label, String value) {
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          Text(label, style: TextStyle(color: textLight.withOpacity(0.8), fontSize: 16)),
