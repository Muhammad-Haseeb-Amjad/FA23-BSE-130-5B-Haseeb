...
class ExportFormatScreen extends StatelessWidget {
  ...
  Widget _buildFormatOption(BuildContext context, {required IconData icon, required String title, required String subtitle, required VoidCallback onTap}) {
    return InkWell(
      onTap: onTap,
      child: Padding(
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text(subtitle, style: TextStyle(color: textLight.withOpacity(0.7))),
                ],
              ),
            ),
            const Icon(Icons.radio_button_off, color: textLight), // Mimic radio button
          ],
        ),
      ),
    );
