...
class RepeatedTasksListScreen extends StatefulWidget {
  ...
  @override
  State<RepeatedTasksListScreen> createState() => _RepeatedTasksListScreenState();
}
...
class _RepeatedTasksListScreenState extends State<RepeatedTasksListScreen> {
  ...
  List<Task> _filterTasks(List<Task> allTasks) {
    ...
    final repeatedTasks = allTasks
        .where((task) => task.repeat != null && task.repeat != 'Does not repeat')
        .toList();
          ...
          child: TextField(
            ...
            decoration: InputDecoration(
              ...
              suffixIcon: IconButton(
                icon: Icon(Icons.clear, color: textColor.withOpacity(0.6)),
                onPressed: () {
                  _searchController.clear();
                  setState(() {
                    _searchQuery = '';
...
// ... (Content Wahi Rahega) ...
  String _getNextRunDate(String? repeatPattern) {
    if (repeatPattern == 'Daily') {
      return DateFormat('EEE, MMM d').format(DateTime.now().add(const Duration(days: 1)));
    }
    if (repeatPattern == 'Weekly') {
      return DateFormat('EEE, MMM d').format(DateTime.now().add(const Duration(days: 7)));
    }
  ...
  Widget build(BuildContext context) {
    ...
    final Color inactiveTrack = theme.brightness == Brightness.dark ? primaryDark : const Color(0xFFC0C0C0);
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      widget.task.title,
                      style: TextStyle( // FIX 9: Text color theme se
                        color: textColor,
                        fontSize: 18,
                    ...
                    Container(
                      ...
                      decoration: BoxDecoration(
                        color: accentColor.withOpacity(0.2), // FIX 10: Accent color theme se
                        borderRadius: BorderRadius.circular(8),
                      ),
                      child: Text(
                        widget.task.repeat ?? 'N/A',
                        style: TextStyle(color: accentColor, fontSize: 12, fontWeight: FontWeight.bold), // FIX 11: Accent color theme se
                      ),
                    ),
                    const SizedBox(height: 8),
                    ...
                    Row(
                      children: [
                        Icon(Icons.access_time, size: 16, color: textColor.withOpacity(0.6)), // FIX 12: Icon color theme se
                        const SizedBox(width: 5),
                        Text(
                          'Next Run: $nextRunDate',
                          style: TextStyle(color: textColor.withOpacity(0.8), fontSize: 14), // FIX 13: Text color theme se
                        ),
                      ],
                    ),
                  ],
              ...
              child: Column(
                ...
                children: [
                  Switch(
                    ...
                    inactiveThumbColor: textColor.withOpacity(0.5), // FIX 15: Thumb color theme se
                    inactiveTrackColor: inactiveTrack, // FIX 16: Track color theme se
                  ),
                  Text(
                    isEnabled ? 'Enabled' : 'Paused', // FIX 5: Text bhi model se
                    style: TextStyle(
                      color: isEnabled ? accentColor : textColor.withOpacity(0.5), // FIX 17: Text color theme se
                      fontSize: 12,
                    ),
                  ),
                ],
